...
# TF-0422 (lot Hoopiz 20260820a, 21/08) : 1920 entre dans les largeurs par défaut — le défaut de
...
() => {
  ...
  // TF-0382 (lot SCC_ALX 20260818b) — trois défauts d'un seul `break`, et le pire n'est pas
  // le plafond.
  //
  ...
  for (const parent of parents) {
    ...
    // TF-0424 (lot Hoopiz 20260820a) : les formes INTERNES d'un groupe SVG titre (<g><title>…)
  ...
  if (window.innerWidth >= __L2_MIN_VIEWPORT__) {
    ...
    for (const el of document.body.querySelectorAll('p, dd, li, blockquote, .va, .prose')) {
      ...
      // TF-0421 (lot Hoopiz 20260820a) : la bride se mesure QUELLE QUE SOIT la propriete.

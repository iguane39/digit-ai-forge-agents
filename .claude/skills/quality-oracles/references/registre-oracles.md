...
> **Vue humaine** (v2.26.1, alignée sur le JSON le 23/09/2026). Source machine (orchestrateur `scripts/run-oracles.mjs`) : `registre-oracles.json`.
...
| Cible mobile : contrat d'usage tactile | `node c:/dev/digit-ai-forge-design/oracles/oracle-mobile.mjs <page.html> --si-cible-mobile` — joué sur les seules cibles mobiles, SANS OBJET ailleurs, règle partagée avec le point d'entrée de forge-design (lib/cible-mobile.mjs, TF-1322, 23/09/2026) — M1 viewport et zoom, M2 cibles ≥ 44 px, M3 safe-area-inset, M4 reflow des tables sous 768 px, M5 orientation paysage, M6 prefers-reduced-motion, M7 prefers-reduced-transparency | cli | ✅ |
